		   ...
		   size_t nInterWorkMemSize)
{
	...
	pInterWorkMem->nESI_max = (int)((nInterWorkMemSize - sizeof(RqInterWorkMem)) / sizeof(uint32_t));
		   ...
		   size_t nInterProgMemSize)
{
	...
	int colperm[n_cols];
	#else
	/*	Because Microsoft Visual C requires static size for array, set max size.
	n_cols = params.L = Kprime + S + H
	The max of n_cols = 56403 + 907 + 16
	*/
	int colperm[56403 + 907 + 16];
	assert(n_cols <= 56403 + 907 + 16);
	#endif
		   ...
		   size_t nInterSymMemSize)
{
	...
	/* Create matrices */
	m256v IB = m256v_make(pcInterProgMem->LU.n_col, (int)nSymSize, pInterSymMem);
	m256v Y = m256v_make(pcInterProgMem->nESI, (int)nSymSize, (void*)pcInSymMem);
		     ...
		     size_t* pOutProgMemSize)
{
	...
	#else
	static_assert(sizeof(RqOutWorkMem) == sizeof(RqOutProgram),
			"RqOutWorkMem and RqOutProgram need to be defined identically.");
	#endif
	      ...
	      size_t nOutWorkMemSize)
{
	...
	const int nESImax = (int)((nOutWorkMemSize - sizeof(*pOutWorkMem)) / sizeof(pOutWorkMem->ESIs[0]));
		 ...
		 size_t nOutSymMemSize)
{
	...
	m256v O = m256v_make(pcOutProgMem->nESI, (int)nSymSize, pOutSymMem);
	m256v I = m256v_make(P->L, (int)nSymSize, (void*)pcInterSymMem);
