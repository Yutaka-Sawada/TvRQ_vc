			...
			const uint32_t* ESIs)
{
	...
	int rowoffs = 0;
	{
		...
		#else
		/*	Because Microsoft Visual C requires static size for array, use malloc.
			This requires free() later. */
		uint32_t *ISIs = malloc(n_ISIs * sizeof(uint32_t));
		#endif
		for (int i = 0; i < n_ESIs; ++i)
			ISIs[i] = ESIs[i] + (ESIs[i] >= (uint32_t)(P->K) ? n_pad : 0);
